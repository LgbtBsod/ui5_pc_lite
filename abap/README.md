...
- Ответ сервера клиент всегда дофильтровывает сам (регистр, `ё`/`е`, все
  слова запроса в любом порядке, по «ФИО + табельный №») и сортирует по
  релевантности; fuzzy-лишние строки отсекаются, поэтому `$orderby` не нужен.
  Поэтому `Pernr` должен быть в `@Search.defaultSearchElement`: клиент ищет и
  по табельному номеру.
- Клиент отправляет текст в нижнем регистре, без схлопывания `ё`/`е`, и
  кэширует ответы по ровно этому тексту. Повторный/уточнённый ввод
  (`иван` -> `иванов`) клиент обслуживает из ответа на более короткий
  запрос только если тот не упёрся в `$top` и после клиентской фильтрации не
  пуст; иначе идёт новый запрос. Предположение: сервер ищет по подстроке
  каждого слова без учёта регистра, так что ответ на короткий запрос —
  надмножество ответа на длинный. Если поиск строго по префиксам слов или
  fuzzy с неполной выдачей, надмножество не гарантировано (пустой итог всё
  равно уходит на сервер). Складывает ли сервер `ё`/`е` — решает сервис:
  клиент этого не предполагает и разные написания не склеивает.
...
- Клиент опирается на `ResultCode` `" "` (одиночный пробел) как код
  «Неудовлетворительно» (см. `model/CheckResults.json`, `BusinessRules.js`).
  Реальный Gateway может сериализовать пустой `CHAR` как `""` — бэкенд/CDS
  обязан вернуть отдельный непустой код либо сохранить пробел.
